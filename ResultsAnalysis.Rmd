...
results_dir_vect <- c(
  "D:\\EcoHab-project\\Results_1_Semaine 1 - Habituation - série 1",
...
grid.arrange(grobs = c(plots_time), ncol = 1)

write.csv(df_time_summary, paste0(results_dir, "\\cleaned_df\\" ,"sollitude.csv"), row.names = FALSE)
...
df_wide_list <- lapply(sociability_files, function(file) {
  ...
  if (grepl("24.00_h", file)) {
    if (grepl("dark", file)) {
      write.csv(df_long, 
                file = paste0(results_dir, "\\cleaned_df\\", "sociability_dark.csv"), 
                row.names = FALSE)
      cat("Archivo guardado para:", file, "\n")
    } else{
      write.csv(df_long, 
                file = paste0(results_dir, "\\cleaned_df\\", "sociability_light.csv"), 
                row.names = FALSE)
      cat("Archivo guardado para:", file, "\n")
    }
  }
