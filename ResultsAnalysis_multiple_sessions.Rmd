...
# Lista de rutas
results_dir <- c(
  "C:\\Users\\sholguin\\Desktop\\EcoHab-project\\Results_1_Semaine 1 - Habituation - série 1",
  "C:\\Users\\sholguin\\Desktop\\EcoHab-project\\Results_Semaine 1 - Habituation - série 2",
  "C:\\Users\\sholguin\\Desktop\\EcoHab-project\\Results_Semaines 4-7 - Pause entre les series de manips"
)
...
zones <- unique(df_visits_summary$zone)
...
zones <- unique(df_time_summary$zone)
plots_time <- list()
...
df_list_solitude <- df_list_all[grepl("sociability", names(df_list_all))]
...
# Normalizar todos los grupos a 10 bins
reduce_to_10_bins <- function(df_group) {
  bins <- 10
  df_group %>%
    mutate(bin = ntile(time_days, bins)) %>%
    group_by(mouse, source_clean, zone, bin) %>%
    summarize(
      time_days = mean(time_days),
      value = mean(value),
      source_df = first(source_df),
      .groups = "drop"
    )
}
library(purrr)

df_groups <- df_long_combined %>%
  group_by(mouse, source_df, zone) %>%
  group_split()

df_reduced_list <- map(df_groups, ~ reduce_to_10_bins(.x))

df_long_reduced <- bind_rows(df_reduced_list)


# Graficar igual que antes
mice <- unique(df_long_reduced$mouse)
zone_palette <- hue_pal()(length(unique(df_long_reduced$zone)))
names(zone_palette) <- unique(df_long_reduced$zone)

library(cowplot)

plots <- list()
for (m in mice) {
  df_mouse <- df_long_reduced %>% filter(mouse == m)

  # Niveles únicos y bloques
  source_df_levels <- unique(df_mouse$source_df)
  n_blocks <- length(source_df_levels)

  # Mapear formas únicas por zona (máx 10 formas)
  zone_levels <- unique(df_mouse$zone)
  shape_vals <- seq_along(zone_levels) %% 25  # 0:24 shapes disponibles en ggplot

  df_mouse <- df_mouse %>%
    mutate(
      source_df = factor(source_df, levels = source_df_levels),
      block = as.numeric(source_df),
      bin_offset = bin + (block - 1) * 10,
      zone = factor(zone, levels = zone_levels)
    )

  p <- ggplot(df_mouse, aes(x = bin_offset, y = value, color = zone, shape = zone)) +
    geom_point(size = 2, alpha = 0.8) +
    scale_color_manual(values = zone_palette) +
    scale_shape_manual(values = shape_vals) +
    labs(
      title = paste("Mouse:", m),
      x = "Bin (offset by condition)",
      y = "Value",
      color = "Zone",
      shape = "Zone"
    ) +
  ...
  # ➤ Líneas de tendencia separadas por bloque
  for (b in seq_len(n_blocks)) {
    df_block <- df_mouse %>% filter(block == b)
    p <- p +
      geom_smooth(
        data = df_block,
        aes(x = bin_offset, y = value, color = zone),
        method = "lm",
        se = FALSE,
        linewidth = 0.8
      )
  ...
  offset_map <- seq(0, by = 10, length.out = n_blocks + 1)
  vlines <- offset_map[-c(1, length(offset_map))]
  for (v in vlines) {
    p <- p + geom_vline(xintercept = v, linetype = "dashed", color = "grey50")
  }

  # ➤ Etiquetas debajo del eje x con ggdraw
